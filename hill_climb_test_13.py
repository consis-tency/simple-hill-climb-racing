...
def updateCar(delta_time):
    ...
    # front_idx = int((car_front_x + terrain_offset_x) / hill_render_step_size)
    # back_idx = int((car_back_x + terrain_offset_x) / hill_render_step_size)
    front_idx = (start_index + end_index)//2
    back_idx = (start_index + end_index)//2
    print(f"Front: {back_idx}, Back: {front_idx}")
    # Ensure the indices are within the bounds of the hills list
    if 0 <= front_idx < len(hills) - 1:
        x1 = front_idx * hill_render_step_size - terrain_offset_x
        y1 = hills[front_idx]
        x2 = (front_idx + 1) * hill_render_step_size - terrain_offset_x
        y2 = hills[front_idx + 1]
    ...
    if 0 <= back_idx < len(hills) - 1:
        x1 = back_idx * hill_render_step_size - terrain_offset_x
        y1 = hills[back_idx]
        x2 = (back_idx + 1) * hill_render_step_size - terrain_offset_x
        y2 = hills[back_idx + 1]
...
def displayDistanceTravelled():
    ...
    glColor3f(0, 0, 0)  # Set text color to black
    renderText(-WINDOW_WIDTH // 2 + 10, WINDOW_HEIGHT // 2 - 60, f"Travelled: {int(terrain_offset_x)}")
